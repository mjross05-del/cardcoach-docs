...
Last updated: 2026-09-15 · Owner: Mike  (header date corrected 2026-07-04, housekeeping sweep 2 — was 2026-07-03, contradicting the 2026-07-04 dated updates within; prior correction 2026-07-03 — was 2026-06-02)
...
- **#49** **Verify engine v2 LIVE 2026-09-08 — cutover has four ★ steps that are Mike's.** Migrations `20260908010442`…`011333` applied, `verify-doc-watch` v2 deployed and proven (TDBank 28/28, Simplii 6/6 + 1 baseline), pg_cron plans at 08:30 UTC and sweeps 08:35–09:55. Cloud runner: Claude Code routine `cardcoach-verify-engine` (`trig_019iW4BgbcU6KS9DpnKQt6Rt`, Card Coach env, 10:30 UTC daily) since 2026-09-11 — the Cowork task `trig_01GsH2snaKFFjrXL8A9YJjdj` is paused after its sandbox lost egress (RUNBOOK §7); sessions 2026-09-08…10 ran on it. Waiting on Mike: (1) read its first digest; (2) `CF_ACCOUNT_ID` + `CF_BROWSER_RUN_TOKEN` function secrets; (3) pause/delete the 8 Cowork batch tasks + `cardcoach-apply-loop` + the retention and DATA-018 triggers; (4) swap the chrome-lane prompt for `PROMPT_engine_runner_chrome.md`. Repos pushed via `PROMPT_deploy_verify_engine_v2_2026-09-07.md`. Section below.
- **#50** **Google Play links on cardcoach.ca — LIVE 2026-09-15 (site `4c4b4e6`, pushed through GitHub's upload page); Mac clean-up pending (Mike's Code session).** The Mac's site repo needs its `main` moved onto GitHub's commit (no push), and monorepo `85401af` plus the docs commit need pushing; `PROMPT_code_push_play_links_2026-09-15.md` does all of it and deletes this note.
...
**2026-09-11 — the cloud runner moved from a Cowork task to a Claude Code routine.** The 10:31 UTC run
(`be081580`) got `gateway answered 403 to CONNECT (organization policy)` on every host — issuers and the evidence
endpoint alike — while `api.github.com` and PyPI still answered: the Trusted allowlist. Nothing on our side had
changed (task last edited 09-09 11:29 UTC; the 09-10 run fetched 17 artifacts with the same `curl`). The task was a
**Cowork cloud task** (`created_kind: cowork_task`) on a platform-managed environment whose egress follows the
account-level Cowork network setting — no per-task control, and it moved under us. The engine behaved as §4 says:
three `fail_work` at attempt 1 (Desjardins Gold, AmexCanada, RBC), `close_run stopped`, zero writes; the 12:30
retry deliberately claimed nothing more. Fix, same day: `trig_01GsH2snaKFFjrXL8A9YJjdj` **paused** (`enabled=false`,
kept); routine **`trig_019iW4BgbcU6KS9DpnKQt6Rt`** created with the same prompt, cron, model and the three
connectors it uses, on the **Card Coach** environment (`env_01GdyJABxXMqRrtEoPgE2t32`) after Mike set **Network
access Full** and a setup script installing `poppler-utils` — the Code image has no `pdftotext`; the first probe
found that. Probes (one-off routines, no engine calls): egress FAIL at Trusted → PASS after the edits (desjardins
200, td 200, evidence-upload 401 = reachable, `pdftotext 24.02.0`); Supabase `execute_sql` works in routine
sessions; the two Cloudflare connectors need re-authorizing (`mcp_auth_required`) — `browser_run` sources only.
First routine run fired 14:43 UTC (`cse_017BAWJcKtNhnXv2Ez49dayC`, engine run `cdd51798`): setup cached,
housekeeping clean (guardrail 0 / unattributed 0), TDBank claimed, 15/15 pages fetched 200, evidence uploads
returning `ok` — digest in the session and the push. RUNBOOK §7 records the environment requirements. ★ item (1)
above now means the routine's digest, not the Cowork task's.

**#49 stays open** — it closes when the ★ items above are done and the routine's first digest
has been read.

## #50 — Google Play links on cardcoach.ca: live; Mac repos to line up (opened 2026-09-15)

**Status: LIVE on cardcoach.ca since ~19:25 ET 2026-09-15** as cardcoach-site `4c4b4e6` (parent
`cd3c330`), verified live. Decision record: `PIPELINE_AND_DECISIONS.md`, entry of 2026-09-15.
Runbook for the Code session: `PROMPT_code_push_play_links_2026-09-15.md`.

**What is left on the Mac:** the site repo's local `main` is `0875e05` — the same tree as GitHub's
`4c4b4e6` (it went up through GitHub's upload page because computer use can't type in Terminal), so
it needs a ref move onto `origin/main`, **not** a push. Monorepo `85401af` (parent `d96b635`) and the
docs commit on `cdf905a` need pushing.

**What closes it:** the prompt's Steps 1–4 green. Then delete this note and its index line.

**Leftovers for the Mac to clear:** stray git lock files from the mount (unlink is refused there) sit in
`CardCoachv2/_to_delete/stale-git-locks-2026-09-15-play-links/` and
`cardcoach-docs/attic/stale-git-locks-2026-09-15-play-links/` — the prompt's Step 0 removes them.
