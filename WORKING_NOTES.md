...
- **#40** **Receipt retention — DECISION NEEDED (Mike).** `receipt.tenants` row `cardcoach` has `retention_days = NULL` (80 receipts, all with images, since 2026-08-26); receipts are keyed by a hash, not a user id, so account deletion does not remove them and per-user deletion is manual. The published policy says so honestly (§2.4). Recommend 90 days; then a session schedules `receipt.purge_expired()` (pg_cron) plus a storage-purge worker for `receipt-images`, and the policy line changes to the fixed period.
- **#41** **Edge deploy of today's changes — Mike, one command** (handoff): `supabase functions deploy` for the twelve functions changed on `main`; `config.toml` now pins the import map so no `--import-map` flag is needed. Until deployed, production runs the 2026-09-02 morning versions (CAPS-001, SEC-002, the webhook fix and the fuel-grade scope are not live).
- **#42** **Undeploy dead functions** — `recommend-here` (v1), `recommend-card` (v1), `resolve-merchant-v1`, `import-spend-v1`, `health` (leaks key generations to any signed-in user). **Not** `recommend-cards-stateless-v1`: cardcoach.ca's /best-card calls it (the review's F-16 listed it as dead — corrected 2026-09-02).
- **#43** **Retired lane's checkout** — `~/dev/CardCoachv2` is still on branch `data-018/fuel-grade-scope` with a working tree main already contains; Mike runs `git stash -u && git checkout main` (handoff), then deletes the branch.
- **#44** **Monday merchant-category batch has never run** (#26) and the p3 `mcc_includes` backfill deltas are still `PENDING_VERIFY` (#27); both change rankings today. Now the review lane's to pick up, in that order.
- **#45** **Company name in the app config** — `app.config.ts` `owner: "falconview"` and the Sentry org `falcon-view-group` predate CardCoach Inc.; the `apps/web` privacy page (unused, FalconView.ai) should be deleted or repointed at the site policy.
