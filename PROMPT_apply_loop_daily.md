...
1. Find unqueued gated results:
   `select fc.* from verify.fact_checks fc left join verify.apply_queue q on q.fact_check_id = fc.id where fc.policy_class='gated' and q.id is null;`
   NOTE — do NOT add `fc.approval_state='pending'` back to that query. `fact_checks.approval_state` is a MIRROR written only by trigger `trg_sync_fact_check_approval`, which fires on `apply_queue` insert/update. A fact_check that was never staged has no queue row, so its mirror was never written and is NULL — and `approval_state='pending' AND q.id IS NULL` is therefore unsatisfiable: the step could never catch anything. `left join ... where q.id is null` alone is the real condition. (2026-08-27: fact_check 8bbac202 sat unstaged and invisible for a day; only `verify.gated_state_guardrail`'s `gated_never_staged` check found it.)
