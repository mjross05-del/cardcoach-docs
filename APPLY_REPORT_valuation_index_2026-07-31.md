...
3. **TD floor fragility** — the 0.25 "Minimum Value" sits in a section TD labels "limited-time promotional offers [we] can cancel or change at any time" (caveat on the row). If that reads too soft for a floor, the alternative is reverting to Expedia 0.50 as conservative.
   **→ RESOLVED 2026-08-01 ~11:35 ET (parallel session, Mike's ruling "do the td conservative open item"): 0.25 STANDS.** Full §3.3 read confirms the caveat attaches to the whole Other Redemption Options category — but td.com's pay-off-purchases page publishes the same $1 = 400 pts as a standing feature ("accurate as of August 19, 2025", no promo framing), attached as a second issuer artifact (delta `2026-08-01__td-conservative__promo-caveat-resolution.sql`). Conservative = lowest channel that exists today; withdrawal is covered by the event-driven refresh triggers, confidence stays `medium`, and the row now carries an explicit revert-to-0.50 instruction if the channel disappears. **All decision items from this report are now closed except item 2 (chart-derived confidence on the RBC Avion rows).** *[Correction ~11:50 ET: stale on arrival — the apply session's ~10:05 ruling had already closed item 2 (Aventura + Avion rows retrofitted to medium-high); verified in-DB by the parallel session. Every item in this report is closed.]* The same session also closed the index's last marriott prereq: PoT Marriott 0.80 CAD read on-page and attached as the fifth source to all three rows (band 0.80–1.00, median still 0.90, values unchanged; delta `2026-08-01__marriott__pot-fifth-source-band-0p80.sql`).
