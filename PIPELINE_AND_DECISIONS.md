...
### 2026-09-08 — The verification batches became an engine: the database is now the program (ENGINE v2, live)
**What happened:** Mike asked for the verify-batch / daily-prompt system to be "more tied to the database and less prone
to failure … an engine that gives CardCoach its competitive advantage … merge it into one entity." The diagnosis, from
89 runs and 1,936 fact checks: the prompts *were* the program (eight hand-copied Cowork batch prompts editable only on
Mike's Mac, `ISSUER_BATCH` hard-coded, render.js and the prompts disagreeing for a week, the apply-loop SKILL mirror
out of sync with its running copy, the chrome lane's scope query matching zero issuers for two weeks); state lived in
prose (`issuer_notes.quirks`, digests composed by hand, counts that could disagree with the tables); coverage was
shallow and unmeasured (no list of which facts the engine is responsible for, so "verified" meant "whatever the
session got to"); budget was burned re-chasing known dead ends every week; vocabulary was free text (`TD`/`TDBank`,
`rbc`/`RBC`, ad-hoc fact keys); every daily run depended on a laptop being awake; and nothing product-facing said how
fresh a card was.
**Decision (Mike, four ★ choices, all "recommended" options taken):** (1) daily runs move to a **cloud** scheduled
task, with client-rendered issuers (RBC, Amex, Canadian Tire, BMO, Neo) rendered through **Cloudflare Browser Run**
over plain HTTPS — headless Chromium is unusable from the cloud sandbox (egress proxy resets the connection), the
extension refuses several issuer domains, and Browser Run was verified against all five; (2) a **mechanical doc-watch**
(`verify-doc-watch` edge function on pg_cron, no LLM) fetches, hashes and flags every registered document daily;
(3) one **work queue owns everything** — issuer verification, change-driven card rechecks, gated staging, proposal
drafting with dry runs, approved applies, chrome-lane captures, the DATA-018 loyalty re-verify and the monthly
snapshot-retention review; (4) a **product-facing freshness surface** exists on the DB side now
(`public.card_verification`: level `issuer_verified` / `partially_verified` / `unverified`, `last_verified_at`,
`facts_fresh`/`facts_total`, RLS read for anon/authenticated) — the app/web read of it is a follow-on lane.
**Principles the schema enforces (not the prompt):** the DB is the program — `verify.plan()` schedules, `claim_work`
leases with `FOR UPDATE SKIP LOCKED` and 90-minute leases that self-heal when a session dies, `work_brief` hands a
runner everything it needs (issuer row, notes, learnings, sources with hashes and channel policy, targets due, known
sourcing gaps, DB values, open queue rows, the active rules); **mechanical before intelligent** (hashing and
reachability need no model); **every fact has a target** (`verify.fact_targets`, synced from `public.*` — fee, FX,
status, name, base earn, each category rate, each cap, the exclusion set, each program's cpp — so coverage is a number,
and three consecutive `unverified` results turn a target into a `sourcing_gap` with a hash-change re-check trigger
instead of a weekly retry); **evidence is a constraint** (`record_fact` refuses a money fact without two artifacts or
two passes, without a grep-guarded quoted clause, with evidence older than 45 days, or with an unknown key/issuer/
topic — the vocabulary tables and their alias tables replace free text); **cpp is never auto** (downgraded and logged);
**auto-class writes are restricted to `card_products` scalars** (fee, FX, base_earn) — earn rows, caps and
`display_name` are gated in v2; **nothing reaches `public.*` except `verify.apply_item` on a row Mike approved through
`verify.decide`**, under a rule 9(a) snapshot, an old-value guard on every UPDATE, a savepoint rollback on any guard
failure (the row flips to `needs_input` with the error), and a `write_audit` row linked to the fact check and the
approver; ledgers (`fact_checks`, `evidence`, `write_audit`, `apply_queue`, `run_events`) are append-only by trigger;
digests are generated by `close_run` from the ledgers, never composed. The runbook rulings survived as data:
grep guard, dual confirmation, Rulings 1–4 on hash and revision date, the 2026-08-26 robots ruling, §10 "not a wall",
the SKILL's apply rules — all rows in `verify.rules` (R01–R25) that every runtime reads on claim; and
`issuer_notes.doc_locations` froze as history, `verify.sources` (396 rows, imported from doc_locations, evidence and
the loyalty stack) is canonical.
**Applied to production 2026-09-08 01:04–01:13 UTC via MCP** (rule 9(e) files under the recorded versions + ledger):
`20260908010442` p1 registry/vocabulary/contract/queue/views · `20260908010931` p2 runner API (39 SECURITY DEFINER
functions, `search_path=''`, service_role only) · `20260908011244` p3 seed + backfill (17 issuers — BMO and Neo moved
to the cloud lane, HSBC off — 22 fact keys, 29 parking topics, 25 rules, schedules per issuer keeping the old weekday
as a preference, done-markers from run history, DATA-018 item due 2027-05-01) · `20260908011333` p4 doc-watch plumbing
(Vault token, four token-checked public RPCs, four pg_cron jobs: plan 08:30, reap hourly, sweeps 08:35–09:55 UTC).
Mike's same-week GUARDRAIL-1 view (`20260907175034`, applied from chat) had no repo file; reconstructed from the
database byte-for-byte and the engine's guardrail view aligned to it. **Tested first** on a faithful local Postgres 16
replica of production (public catalogue + verify ledgers): thirteen intentional refusals (unknown key, stale evidence,
money fact without dual confirmation, cpp auto, DELETE on a ledger, `runtime_flags` in a proposal, UPDATE without a
guard, …) and the happy paths (auto scalar with snapshot + audit, gated stage → decide → dry run [1,1] → apply →
audit linked to the fact check, lease reaping, plan idempotence).
**State at cutover:** 152 active cards; 1,551 active targets — 224 fresh, 854 never verified (the honest coverage
number the old system never produced); 31 sourcing gaps identified automatically from history (Amex FX ×13, RBC ×6,
MBNA ×6, CIBC student fees ×5, TD ×1); 396 sources, 133 hash-baselined; card levels 123 `partially_verified` /
30 `unverified` / 0 `issuer_verified`; guardrail 0; health clean but `issuers_overdue`. `verify-doc-watch` **v2**
deployed and proven the same night: wrong token → 401; TDBank sweep 28/28 fetched, 0 changed (every stored hash
matched the live PDF — the legacy hashing and the engine's agree); SimpliiFinancial 6/6 with one agreement baselined
into Storage + `verify.evidence`. First `verify.plan()` queued 16 doc-watch items, BMO + NeoFinancial issuer items,
one loyalty re-verify and the housekeeping kinds.
**What changes for Mike:** review is two statements in any chat with the Supabase connector —
`select * from verify.v_review_packet;` then `select verify.decide('<queue_id>', 'approved'|'rejected', 'mike', 'why');`
(dry-run row counts sit on the row). Behaviour changes are rows, not prompt edits (cadence, preferred day, rules,
sources, gaps, fact kinds, lane moves — RUNBOOK_verify_engine_v2 §3). Status is `verify.v_engine_dashboard` /
`v_engine_health` / `v_freshness_by_issuer` / `v_next_work`. The cloud task `cardcoach-verify-engine` (`trig_01GsH2snaKFFjrXL8A9YJjdj`, 10:30 UTC daily,
`PROMPT_engine_runner_cloud.md`) is created and enabled; its first session is 2026-09-08. **Cutover steps still Mike's:** set `CF_ACCOUNT_ID` +
`CF_BROWSER_RUN_TOKEN` as function secrets (until then rendered pages are `skipped` by the doc-watch and rendered by
the runner through the connector); pause/delete the eight Cowork batch tasks + `cardcoach-apply-loop` (both systems
running would double-verify, and the legacy prompts now hit the vocabulary trigger on any new spelling) and the
retention / DATA-018 triggers (work items now); swap the chrome-lane prompt for `PROMPT_engine_runner_chrome.md`;
push both repos (`PROMPT_deploy_verify_engine_v2_2026-09-07.md`). WORKING_NOTES #49.
**Not done, on purpose:** no app/web UI reads `card_verification` yet; the legacy runbooks stay as the record of the
