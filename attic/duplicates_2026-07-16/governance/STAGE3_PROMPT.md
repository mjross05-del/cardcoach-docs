...
**Version 1.1 · Last updated 2026-04-22**
...
structured, field-level delta for Alex to apply as SQL.
...
## Open questions for Alex (resolve before the Scotia Momentum dry run)

These would otherwise force a v1.2 immediately after the first run. **Also tracked in
`WORKING_NOTES.md` #1 — keep them in sync, but this is the technical detail.**

1. **RLS write-path asymmetry.** `card_caps` and `card_exclusions` have explicit `_service`
   policies for `service_role`. `card_products` and `earn_rates` do not — they rely on
   `service_role` BYPASSRLS. Should the SQL handoff assume service-role execution for all
   four tables, or a different path?
2. **SQL file format** for approved deltas — one file per card per run, or per table?
3. **Filename convention and transaction wrapping** (BEGIN/COMMIT, savepoints, idempotent
   guards).
4. **Audit metadata placement** — SQL comments inline, or a separate `.md` sidecar.
5. **HSBC handling** — is `application_status = 'closed'` the right semantics, or does Alex
   want `is_active = false`? Schema allows both; different implications for what appears in
   exports.
