...
**Rider R2 — FK observation (verify batch, 2026-08-11):** Observed 2026-08-11 during probe-user cleanup: `user_cards` → `auth.users` cascades on delete; `user_loyalty_links` does not — orphan row cleaned manually. App-side account deletion would orphan loyalty links. Observation only; no fix applied.
